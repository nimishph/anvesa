...
// a per-platform package (`@sutras/code-lens-<os>-<cpu>`), installed alongside this one because
...
function platformPackage(platform, cpu) {
  ...
  return SUPPORTED.includes(key) ? `@sutras/code-lens-${key}` : undefined;
}
  ...
  try {
    ...
    return {
      problem:
        ...
        'It is an optional dependency of @sutras/code-lens: reinstall without --no-optional, or ' +
